...
/**
 * Mapa producto -> flujos que le pertenecen, leído del grafo del agente (builder V2).
 *
 * Un flujo "es de" un producto si, partiendo del nodo `producto`, se llega a un nodo `flujo`
 * caminando hacia adelante por las aristas (producto → condiciones → flujo). Es la MISMA
 * traversía que usa `resolveFlowBranchForActiveProduct` para disparar la rama del producto activo.
 *
 * Se usa para el candado de un-solo-producto en `enviar_flujo`: sirve para saber si un flujo que
 * la IA quiere mandar pertenece a OTRO producto distinto del que se está conversando (p.ej.
 * manicura en una charla de camillas), y así frenarlo y pedir confirmación en vez de mandarlo.
 *
 * IMPORTANTE (para no atribuir mal): la caminata NO cruza a través de otro nodo `producto`; los
 * flujos que cuelgan de otro producto son de ESE producto, no de este. Devuelve productId -> Set<flowId>.
 */
export function collectFlowIdsByProduct(graph: unknown): Map<string, Set<string>> {
  const result = new Map<string, Set<string>>();
  const g = graph as { nodes?: FlowBranchNode[]; edges?: FlowBranchEdge[] } | null;
  ...
  const productNodes = nodes.filter((n) => n.type === "producto" && fbStr(n.data?.productId));

  for (const productNode of productNodes) {
    const productId = fbStr(productNode.data?.productId);
    const flowIds = result.get(productId) ?? new Set<string>();

    const visited = new Set<string>();
    const queue: string[] = [productNode.id];
    while (queue.length) {
      ...
      if (node?.type === "flujo") {
        const flowId = fbStr(node.data?.flowId);
        if (flowId) {
          flowIds.add(flowId);
        }
      }
      ...
      for (const edge of edges) {
        ...
        // No cruzar hacia otro producto: sus flujos le pertenecen a ESE producto.
        if (nodeById.get(edge.target)?.type === "producto") {
          continue;
        }
