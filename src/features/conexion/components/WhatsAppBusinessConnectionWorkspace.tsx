...
import { RecuperarTelefonosCard } from "./RecuperarTelefonosCard";
...
export function WhatsAppBusinessConnectionWorkspace({
      ...
      <CardContent className="relative flex flex-row items-center justify-between gap-3 py-3.5 pl-5">
        <div className="flex min-w-0 items-center gap-3">
          {/* Mismo punto de estado que en la lista de canales: se entra aca desde ahi, asi que
              ...
              distingue "Esperando QR"), por eso son dos colores y no tres. */}
          <span className="relative shrink-0">
            ...
            <span
              className={`absolute right-0 bottom-0 size-3.5 rounded-full ring-2 ring-card ${
                connectionStatusLabel === "Conectado" ? "bg-emerald-500" : "bg-muted-foreground/40"
              }`}
  ...
  const agentTabContent = (
    <div className="grid gap-4 md:grid-cols-2">
      ...
      {connection.provider === "EVOLUTION" ? <RecuperarTelefonosCard /> : null}
