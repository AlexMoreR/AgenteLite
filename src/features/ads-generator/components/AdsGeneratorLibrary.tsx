...
export function AdsGeneratorLibrary({ history }: AdsGeneratorLibraryProps) {
  ...
  return (
    <section className="app-page space-y-6">
      <div className="flex flex-col gap-4 rounded-2xl border border-border bg-card px-5 py-5 shadow-lg md:flex-row md:items-center md:justify-between md:px-7">
      ...
      {sortedHistory.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-card px-6 py-12 text-center shadow-lg">
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-lg">
          <div className="hidden grid-cols-[minmax(240px,1.2fr)_minmax(220px,1fr)_140px_180px] gap-4 border-b border-border bg-muted/80 px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground md:grid">
            <span>Producto</span>
            <span>Titulo</span>
            <span>Creado</span>
          ...
          <div className="divide-y divide-border">
            {sortedHistory.map((entry) => (
              <article
                key={entry.id}
                className="px-4 py-4 transition hover:bg-muted/60 md:px-5"
              >
                <div className="grid gap-4 md:grid-cols-[minmax(240px,1.2fr)_minmax(220px,1fr)_140px_180px] md:items-center">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 line-clamp-2 whitespace-pre-line text-sm leading-5 text-muted-foreground md:hidden">
                  ...
                  <div className="flex items-center gap-2 md:justify-end">
                    ...
                    <Button
                      type="button"
                      variant="outline"
                      className="h-11 w-11 rounded-full p-0 text-destructive hover:text-destructive"
      ...
      {modalOpen ? (
        ...
        >
          <div
            className="my-auto w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-lg sm:p-6"
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                type="button"
                className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-border bg-muted text-muted-foreground transition hover:bg-muted"
                onClick={() => {
                  if (pendingCreate) {
                    return;
                  }
            ...
            <div className="mt-5 space-y-2">
              <label htmlFor="new-ad-product-name" className="text-sm font-medium text-foreground">
                Nombre del producto
              </label>
              <input
                ...
                className="h-12 w-full rounded-2xl border border-border bg-card px-4 text-sm text-foreground outline-none transition focus:border-primary"
                disabled={pendingCreate}
              />
            </div>
