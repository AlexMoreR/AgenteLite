...
const baseProductSchema = z.object({
  ...
  // El CRM solo maneja el PRECIO DE VENTA final. El costo y los margenes se llevan en el
  // sistema de Gestion, no aqui: quedan opcionales (default 0) para no romper la BD ni el
  // agente, pero ya no se piden en el formulario.
  baseCost: z.coerce.number().min(0, "El costo no puede ser negativo").optional().default(0),
  retailMarginPct: z.coerce.number().min(0).max(1000).optional().default(0),
  retailPrice: z.coerce.number().positive("El precio de venta debe ser mayor que 0"),
  wholesaleMarginPct: z.coerce.number().min(0).max(1000).optional().default(0),
  wholesalePrice: z.coerce.number().min(0).optional().default(0),
  minWholesaleQty: z.coerce.number().int().min(1).max(100000).optional().default(1),
...
export async function adminCreateProductAction(formData: FormData): Promise<void> {
  ...
  const parsed = createProductSchema.safeParse({
    ...
    retailMarginPct: formData.get("retailMarginPct") || undefined,
    retailPrice: formData.get("retailPrice"),
    wholesaleMarginPct: formData.get("wholesaleMarginPct") || undefined,
    wholesalePrice: formData.get("wholesalePrice") || undefined,
    minWholesaleQty: formData.get("minWholesaleQty") || undefined,
