  ...
  return (
    <OfficialApiPanelShell basePath={basePath}>
      ...
      <div className="flex min-h-[calc(100dvh-8rem)] flex-1 flex-col gap-0 md:min-h-0 md:grid md:grid-cols-[380px_minmax(0,1fr)]">
        <Card className={`${hasMobileSelection ? "hidden md:flex" : "flex"} min-h-0 flex-1 overflow-hidden border border-border bg-card p-0 shadow-sm md:h-[76vh] md:flex-none`}>
            ...
            <div className="shrink-0 border-b border-border bg-card px-3 py-3">
              <form className="relative" action={`${basePath}/chats`}>
                ...
                <input
                  ...
                  className="h-10 w-full rounded-2xl border border-input bg-muted pl-9 pr-3 text-sm text-foreground outline-none transition focus:border-primary focus:bg-background"
                />
              </form>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto divide-y divide-border">
              {data.conversations.length > 0 ? data.conversations.map((conversation) => {
                ...
                return (
                  ...
                  >
                    <span className={`absolute inset-y-3 left-0 w-1 rounded-r-full ${isSelected ? "bg-primary" : "bg-transparent"}`} />
                    <div className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-muted text-sm font-semibold text-foreground">
                      {getInitials(label)}
                    </div>
                    <div className="min-w-0 overflow-hidden space-y-0.5">
                      <div className="flex items-center justify-between gap-3">
                        <p className="min-w-0 flex-1 truncate text-[13px] font-semibold text-foreground">{label}</p>
                      ...
                      <div className="flex min-w-0 max-w-full items-center gap-2 overflow-hidden">
                        ...
                        <p className="block min-w-0 flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-[13px] text-muted-foreground">
                ...
                <div className="px-5 py-12 text-center">
                  <div className="mx-auto flex max-w-sm flex-col items-center gap-3">
                    ...
                    <div className="space-y-1">
                      <h3 className="text-base font-semibold text-foreground">Aun no hay conversaciones</h3>
                      <p className="text-sm leading-6 text-muted-foreground">Cuando lleguen mensajes por WhatsApp en la API oficial, apareceran aqui.</p>
                    </div>
                  </div>
                </div>
              )}
        ...
        <Card className={`${hasMobileSelection || data.conversations.length === 0 ? "flex" : "hidden md:flex"} min-h-0 flex-1 overflow-hidden border border-border bg-card p-0 shadow-sm md:h-[76vh] md:flex-none`}>
          {data.selectedConversation ? (
            <div className="flex min-h-0 h-full w-full flex-1 flex-col">
              <div className="sticky top-0 z-10 shrink-0 border-b border-border bg-card px-3 py-3 md:static md:px-2.5 md:py-2.5">
                <div className="flex min-w-0 items-center justify-between gap-4">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-muted text-sm font-semibold text-foreground">
                        ...
                        <div key={message.id} className="space-y-3">
                          {showDateDivider ? (
                            <div className="flex justify-center">
                              <span className="rounded-full border border-border/70 bg-card/80 px-3 py-1 text-xs font-medium capitalize text-muted-foreground shadow-sm backdrop-blur">
                                {formatDateDivider(message.createdAt)}
                              </span>
                            </div>
                          ) : null}
                          <div className={`flex ${outbound ? "justify-end" : "justify-start"}`}>
                            <div className={`max-w-[85%] rounded-xl px-2.5 py-[9px] text-[13px] leading-5 shadow-sm md:max-w-[72%] ${
                              outbound ? "bg-primary text-primary-foreground" : "border border-border bg-card text-foreground"
                            }`}>
                              <p>{message.content || "-"}</p>
                              <div className={`mt-1.5 flex items-center justify-end gap-1 text-[10px] ${outbound ? "text-primary-foreground/80" : "text-muted-foreground"}`}>
            ...
            <div className="flex min-h-[74vh] items-center justify-center px-6 py-10 text-center">
              <div className="mx-auto flex max-w-sm flex-col items-center gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm leading-6 text-muted-foreground">Elige un chat de la columna izquierda para ver el historial.</p>
                </div>
              </div>
            </div>
          )}
